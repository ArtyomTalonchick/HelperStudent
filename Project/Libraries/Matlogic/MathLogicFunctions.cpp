...
extern "C"  __declspec(dllexport) char* CheckFunctionMonotone(int n, int arg)
{
	int  temp, size = 0;
	char *p;
	p = checkInput(n, arg);
...
extern "C"  __declspec(dllexport) char* CheckFunctionSavesZero(int n, int arg)
{
	int  temp, size = 0;
	char *p;
	p = checkInput(n, arg);
	...
	reverse(a.begin(), a.end());
	if(a[0] == 0)
		return "Сохраняет ноль";
	return "Не сохраняет ноль";
}

extern "C"  __declspec(dllexport) char* CheckFunctionSavesOne(int n, int arg)
{
	int  temp, size = 0;
	char *p;
	p = checkInput(n, arg);
	...
	reverse(a.begin(), a.end());
	if(a[count - 1] == 1)
		return "Сохраняет единицу";
	return "Не сохраняет единицу";
}
//---------------------------------------------------------------------------
